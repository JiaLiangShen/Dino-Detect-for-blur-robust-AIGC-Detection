#!/usr/bin/env python3
"""
CCMBA预处理脚本 - 简化版（支持多类别数据集结构）
专为tstrain.py优化，只生成必要的文件
支持新的数据集结构：root/category/0_real, root/category/1_fake
"""
...
class CCMBAPreprocessor:
    ...
    def process_category_class(self, input_dir, output_dir, category, class_name):
        """处理特定类别的特定类（real/fake）"""
        input_path = Path(input_dir)
        # 输出路径结构：output_dir/category/nature 或 output_dir/category/ai
        class_folder_name = "nature" if class_name == "real" else "ai"
        output_path = Path(output_dir) / category / class_folder_name
        ...
        print(f"Processing {len(image_files)} {class_name} images from {category} category...")
        ...
        for img_path in tqdm(image_files, desc=f"Processing {category}/{class_name}"):
        ...
        print(f"✓ {category}/{class_name} processing completed:")
        print(f"  - Successful: {stats['processed_successfully']}")
        print(f"  - Failed: {stats['failed']}")
        
        return stats
    
    def process_multi_category_dataset(self, root_dir, output_dir):
        """处理多类别数据集"""
        ...
        if not categories:
            print(f"No valid categories found in {root_dir}")
            print("Expected structure: root/category/0_real, root/category/1_fake")
            return {}
        ...
        for category in categories:
            ...
            category_stats = {'real': None, 'fake': None}
            ...
            # 处理真实图像
            real_path = category_path / "0_real"
            if real_path.exists():
                real_stats = self.process_category_class(
                    real_path, output_dir, category, "real"
                )
                category_stats['real'] = real_stats
                total_processed += real_stats['processed_successfully']
                total_failed += real_stats['failed']
            
            # 处理虚假图像
            fake_path = category_path / "1_fake"
            if fake_path.exists():
                fake_stats = self.process_category_class(
                    fake_path, output_dir, category, "fake"
                )
                category_stats['fake'] = fake_stats
                total_processed += fake_stats['processed_successfully']
                total_failed += fake_stats['failed']
        ...
        summary_path = Path(output_dir) / 'ccmba_multi_category_summary.json'
...
def main():
    parser = argparse.ArgumentParser(description='CCMBA Preprocessing - Multi-Category Support')
    
    # 支持两种模式：旧模式（分别指定real/fake目录）和新模式（指定根目录）
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--root_dir', help='Root directory with multi-category structure (new mode)')
    group.add_argument('--legacy_mode', action='store_true', 
                       help='Use legacy mode with separate real/fake directories')
    
    # 旧模式参数（兼容性）
    parser.add_argument('--real_dir', help='Real images directory (legacy mode)')
    parser.add_argument('--fake_dir', help='Fake images directory (legacy mode)')
    ...
    print("="*70)
    print("CCMBA PREPROCESSING PIPELINE (MULTI-CATEGORY SUPPORT)")
    print("="*70)
    
    if args.root_dir:
        # 新模式：多类别数据集
        print(f"Processing multi-category dataset from: {args.root_dir}")
        print(f"Expected structure: root/category/0_real, root/category/1_fake")
        
        stats = processor.process_multi_category_dataset(args.root_dir, args.output_dir)
        ...
        if stats:
            ...
            print(f"Total categories processed: {stats['summary']['total_categories']}")
            print(f"Total images processed: {stats['summary']['total_processed']}")
            print(f"Total failed: {stats['summary']['total_failed']}")
            
            # 显示每个类别的统计
            for category, category_stats in stats['categories'].items():
                real_count = category_stats['real']['processed_successfully'] if category_stats['real'] else 0
                fake_count = category_stats['fake']['processed_successfully'] if category_stats['fake'] else 0
                print(f"  - {category}: {real_count} real, {fake_count} fake")
    
    elif args.legacy_mode:
        # 旧模式：分别指定real/fake目录（向后兼容）
        if not args.real_dir or not args.fake_dir:
            print("Error: Legacy mode requires both --real_dir and --fake_dir")
            return
        
        print(f"Processing legacy dataset:")
        print(f"  Real: {args.real_dir}")
        print(f"  Fake: {args.fake_dir}")
        
        # 使用旧的处理方法
        real_stats = processor.process_category_class(args.real_dir, args.output_dir, 'legacy', 'real')
        fake_stats = processor.process_category_class(args.fake_dir, args.output_dir, 'legacy', 'fake')
        ...
        summary_path = Path(args.output_dir) / 'ccmba_legacy_summary.json'
        with open(summary_path, 'w') as f:
        ...
        print("="*70)
        print(f"Output directory: {args.output_dir}")
        print(f"Total processed: {total_stats['summary']['total_processed']}")
